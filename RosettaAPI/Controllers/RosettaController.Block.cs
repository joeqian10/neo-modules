...
namespace Neo.Plugins
{
    partial class RosettaController
    {
        ...
        /// <returns></returns>
        [HttpPost("/block")]
        public IActionResult Block(BlockRequest request)
        {
            var index = request.BlockIdentifier.Index;
            var hash = request.BlockIdentifier.Hash;
            if (index == null && hash == null)
                return FormatJson(Error.BLOCK_IDENTIFIER_INVALID.ToJson());
            if (index != null && index < 0)
                return FormatJson(Error.BLOCK_INDEX_INVALID.ToJson());
            ...
            if (hash != null)
            {
                if (!UInt256.TryParse(hash, out blockHash))
                    return FormatJson(Error.BLOCK_HASH_INVALID.ToJson());
            }
            else
            {
        ...
        public IActionResult BlockTransaction(BlockTransactionRequest request)
        {
